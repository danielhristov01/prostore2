...
import { formatError, mapOrder } from "../utils";
...
export async function createOrder() {
  try {
    ...
    //Create a transaction to create order and order intems in database
...
export async function getMyOrders({
  ...
  const session = await auth();
  if (!session) throw new Error("User in not authorized");
